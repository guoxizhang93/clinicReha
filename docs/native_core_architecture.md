...
ClinicCore.dll
  - CAN/UART 帧封装与电机状态解码
  - 电机属性写入、角度预置、执行、阻抗控制、自适应力位、速度/扭矩、PID、角度范围等高层命令帧构造
  - 四杆机构角度转换
  - 训练状态机与轨迹计算
  - OpenSignal 样本解析
  - 推杆/气囊反馈解析
...
C# 不再保留 MATLAB 算法 fallback。`ClinicCore.dll` 缺失、位数不匹配或导出函数缺失时，核心功能直接报错；设备无硬件或端口未打开时明确失败，不再进入设备仿真模式。
...
- `clinic_motor_set_angle_frame`
- `clinic_motor_step_execute_frame`
- `clinic_motor_preset_speed_frame`
- `clinic_motor_preset_torque_frame`
- `clinic_motor_motion_aid_frame`
- `clinic_motor_read_property_frame`
- `clinic_motor_adaptive_angle_frame`
- `clinic_motor_adaptive_multi_execute_frame`
- `clinic_motor_set_speed_frame`
- `clinic_motor_set_torque_frame`
- `clinic_motor_pid_property_frames`
- `clinic_motor_angle_range_frames`
- `clinic_motor_config_order_frame`
- `clinic_motor_init_config_frame`
